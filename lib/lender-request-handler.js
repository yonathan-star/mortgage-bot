...
function extractLoanFromSubject(subject) {
  ...
  const loanFirst = trimmed.match(/^(\d{6,})\s*[-–]\s*(.+)$/);
  if (loanFirst) return { borrower: borrowerHint.borrower, loanNumber: loanFirst[1] };
...
function extractRequestText(body) {
  ...
  for (const line of lines) {
    ...
    if (/^(thank|thanks|regards|sincerely|best|have a great|description automatically|thank you in advance)/i.test(line)) break;
